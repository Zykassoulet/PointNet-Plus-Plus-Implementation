...
shape_net_part_2015_dir = os.path.join( script_dir, '..', 'Datasets','ShapeNetPart2015','PointCloudDatasets' )
...
class Group:
    def __init__(self,center, coords, features):     # data_points : ensemble des points dans le groupe
        self.center = center         #coordonnées du centre du groupe ::tensor(1,d)
        self.coords = coords - center       #coordonnées des points centré en 0 coords ::tensor(K,d)
...
def FPS(point_cloud, k):
    ...
    N = point_cloud.size()[0]
    D = torch.zeros((N,N), dtype=torch.float) # An NxN distance matrix for points
...
def ball_grouping(center, data_points,r):     
    distances = torch.linalg.vector_norm(data_points.coords[:,:3] - center[:,:3],dim=1)
    group = Group(center, data_points.coords[distances<r], data_points.features[distances<r])
    return group
    
    
def grouping_layer(centers, data_points,grouping_method,parameter):        #centers :  tensor (N,d), points_data : all the data in one tensor Nx(d+C)
    grouping = []
    for i in range(centers.shape[0]):
        grouping.append(grouping_method(centers[i:i+1,:], data_points, parameter))
    return grouping         #liste de N' tensor de chacun Kx(d+C)  (K variable selon les groupes)
...
class Pointnet(torch.nn.Module):  # N’xKx(d+C) -> N’x(d+C’)
    ...
    def process(self,groups):     # groups : liste de N' de groupe de Kx(d+C)
        ...
        for j in range(len(groups)):
            ...
            features.append(self(torch.cat((group.coords,group.features),dim=1)))
...
class SA(torch.nn.Module):
    ...
    def abstract(self,data_points):
        ...
        centers = ops.sample_farthest_points(coords[None,:],point_cloud_size,self.K)[0].squeeze()
        if cuda:
...
class SA_end(torch.nn.Module):
    ...
    def abstract(self,data_points):
        final_group = Group(data_points.coords[0:1,:],data_points.coords,data_points.features)  #groupe englobant tous les points restants
        groups = [final_group]
        next_data_points = self.pointnet.process(groups)
        return next_data_points
...
class Abstract_net(torch.nn.Module):
    def __init__(self,ifd,SA_configs,SA_end_config):
        ...
        self.sdp = [0 for  i in range(len(SA_configs)+2)]               #data_points successifs
        self.SA = []
...
def k_means(k, p, data_points, points_coords):      #points_coords : tensor(N,d) , data_points : N'x(C+d)
    ...
    distances = torch.linalg.vector_norm(data_points_coords-points_coords[:,None,:], dim = 2)  #NxN'
...
class Seg_lay(torch.nn.Module):
    ...
    def forward(self,from_points, to_points):        #from_points : data_points interpolé, to_points : data_points vers lesquels on interpole
        res = torch.cat((to_points.features,k_means(self.k, self.p, from_points,to_points.coords)),dim = 1)
...
class Seg_net(torch.nn.Module):
    def __init__(self,k,p,inp_dims,FP_configs):
        ...
        for i, FP_config in enumerate(FP_configs):
            self.seg_lays.append(Seg_lay(k,p,inp_dims[i],FP_config, i == len(FP_configs)-1))
...
class PointNetpp(torch.nn.Module):
    ...
    def forward(self,data_points):
        sdp = self.abstract_net(data_points)       #successive data points calculated by abstract_net
    ...
    def draw(self):
        ...
        for i,label in enumerate(self.labels):
            for v_lbl in self.values:
                ...
                plt.plot(self.x_values[:n_values], self.values[v_lbl][i],Plot.colors[v_lbl],label = self.labels[i] + '_' + v_lbl)
        plt.legend()
...
def net_accuracy_and_loss(loader,net, n_max=64):
    ...
    for data in loader:
        ...
        res_data_points_features = torch.argmax(res_data_points.features,dim = 1)
        accuracy = torch.mean(torch.eq(res_data_points_features, categories).float())
...
def load_shape_net_2015_data(cuda = False):
    ...
    shape_net_2015_dataset = dataset.Dataset(root=shape_net_part_2015_dir, dataset_name=dataset_name, num_points=2048, split='train',segmentation = True)
    train_loader = []
    for data in shape_net_2015_dataset:
        if data[1][0].numpy() in class_list:
            coords, features, lbl = data[0], torch.empty((data[0].shape[0],0)),data[2]
            if cuda:
                coords, features, lbl = coords.type(torch.cuda.FloatTensor), features.type(torch.cuda.FloatTensor), lbl.type(torch.cuda.LongTensor)
            data_points = Data_points(coords,features)
            data = [data_points,lbl]
            train_loader.append(data)
        
    shape_net_2015_dataset = dataset.Dataset(root=shape_net_part_2015_dir, dataset_name=dataset_name, num_points=2048, split='val',segmentation = True)
    valid_loader = []
    for data in shape_net_2015_dataset:
        if data[1][0].numpy() in class_list:
            coords, features, lbl = data[0], torch.empty((data[0].shape[0],0)),data[2]
    ...
    shape_net_2015_dataset = dataset.Dataset(root=shape_net_part_2015_dir, dataset_name=dataset_name, num_points=2048, split='test',segmentation = True)
    test_loader = []
    for data in shape_net_2015_dataset:
        if data[1][0].numpy() in class_list:
            coords, features, lbl = data[0], torch.empty((data[0].shape[0],0)),data[2]
...
res_data_points_features = torch.argmax(point_net_pp(used_data_points).features,dim = 1)
